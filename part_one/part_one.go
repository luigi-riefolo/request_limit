...
var (
	...
	// [1 B] Used to handle concurrent requests.
	requestChan = make(chan struct{}, maxRequests)
)
...
func printRequestNo(w http.ResponseWriter, r *http.Request) {
	...
	// [1 B] Start processing the request
	// or wait until the buffer is not full.
	requestChan <- struct{}{}

	// [1 B] Free the buffer space
