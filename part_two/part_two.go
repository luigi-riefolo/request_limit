...
var (
	...
	reqCnt requestCounter
	// Map containing the available resources.
	mux map[string]func(http.ResponseWriter, *http.Request)
	// Remote IPs requesting the counter value.
	currIPReq = make(map[string]int)
)
...
func printRequestNo(w http.ResponseWriter, r *http.Request) {
	...
	// [2] Check if the IP has exceeded the
	// number of maximum concurrent requests.
	hasIPexceeded := func() bool {
		reqCnt.cntMutex.Lock()
		defer reqCnt.cntMutex.Unlock()
		if IPreqCnt, ok := currIPReq[IP]; ok && IPreqCnt >= maxRequests {
			errMsg := fmt.Sprintf("Maximum number of concurrent requests exceeded '%d'\n",
				maxRequests)
			// [2] Reject any additional request
			http.Error(w, errMsg, http.StatusTooManyRequests)
			return true
		}
		return false
	}
	if hasIPexceeded() == true {
		return
	}
	...
	// [2] Increment the current IP's counter
	currIPReq[IP]++

	reqCnt.timeLapseReqNo++
	...
	// [2] Decrement current IP's counter
	reqCnt.cntMutex.Lock()
	currIPReq[IP]--
	reqCnt.cntMutex.Unlock()
}
